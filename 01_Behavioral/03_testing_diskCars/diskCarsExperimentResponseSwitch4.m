...
function trialOutput = diskCarsExperimentResponseSwitch(name,exptdesign)
...
% %     end
    ...
    for sessionNum = 1:exptdesign.numSessions;
        ...
        tic;
%         if sessionNum == 1
%             drawAndCenterText(w, 'Welcome \n \n \n  Setting up session, please wait... \n \n \n \n Press "S" if the center is a SOVOR \n and "D" if it is a ZUPUD. \n \n Please respond as quickly and as accurately as possible',0);
%         else
...
%         end
        ...
        if displayCentral
             % PICTURES
            [centallimages, centstimulustypemarker] = prepCentPics(exptdesign);
...
% % %         letterF = imread(exptdesign.Fgraphic);
        ...
        %determine whether to display central and/or peripheral task
        if exptdesign.sessionType(sessionNum) == 1
            drawAndCenterText(w, 'Sovor/Zupud \n\n Press Left for SOVOR. \n Press Right for ZUPUD.  \n\n (press key to begin)...');
        elseif exptdesign.sessionType(sessionNum) == 2
            drawAndCenterText(w, 'Disk: Red on Left (press 1) or Right (press 2)? \n\n (press key to begin)...');
        elseif exptdesign.sessionType(sessionNum) == 3
            drawAndCenterText(w, 'Disks and Cars simultaneously \n\n First respond to disk (Red on left (press 1) or right (press 2)). \n Second respond to the car (Sovor - Left or Zupud - Right). \n Always attend to the DISK.  \n\n (press key to begin)...');
        end
...
%             end
        ...
        tic;
        save(['./Car_data/' exptdesign.subjectName '/' exptdesign.subjectName '.' num2str(sessionNum) '.' num2str(trial) '.mat'],'trialOutput','exptdesign');
        toc;
...
%           stimulustypemarker - tells the function whether each image is a
%           target or a distractor
function [centallimages, centstimulustypemarker] = prepCentPics(exptdesign)
...
%randomly choose a value between 1-10
jitter=round(1+9*rand(1));

%allimages(:,:,:,:) = cat1Images(:,:,:,:);
%allimages(exptdesign.numCat1PerSession+1:exptdesign.numCat1PerSession+exptdesign.numCat2PerSession,:,:,:) = cat2Images(:,:,:,:);
allimages=[line1images([0:10:80]+jitter,:,:); line2images([0:10:80]+jitter,:,:); line3images([0:10:80]+jitter,:,:); line4images([0:10:80]+jitter,:,:); ...
    line1images([120:10:200]+jitter,:,:); line2images([120:10:200]+jitter,:,:); line3images([120:10:200]+jitter,:,:); line4images([120:10:200]+jitter,:,:)];
