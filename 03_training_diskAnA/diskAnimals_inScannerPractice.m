...
exptdesign.numTrialsPerSession = 16;     % number of trials per session (each session is roughly 5 minutes)
...
for run = 1:size(exptdesign.sessionTypes,1)
    ...
    exptdesign.run=run;
    [trialoutput] = diskAnimalsExperiment_inScanner(name,exptdesign);
end
