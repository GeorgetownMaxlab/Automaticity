...
function trialOutput = diskAnimalsTrainingExperiment20141028(name,exptdesign)
...
%             end
        ...
        if mod(sessionNum,3)==0
            ...
            if max(centPerformance)>0.85
                exptdesign.centSOADurations = max(exptdesign.centSOADurations - exptdesign.refresh*2, exptdesign.refresh*2);
            end
            
            if max(perifPerformance)>0.85
                exptdesign.perifSOADurations = max(exptdesign.perifSOADurations - exptdesign.refresh*2, exptdesign.refresh*2);
            end
